...
https://sic2027vxwebyc.github.io/VX_WEB_DEMO/
...
*   **Vite PWA Plugin**: `vite-plugin-pwa`를 활용하여 서비스 워커 생성, 매니페스트 관리, 캐싱 전략을 자동화했습니다.
...
3.  **이벤트 참여**: 홀 곳곳에 숨겨진 QR 코드를 찾아 스캔하여 **Passport**에 스탬프를 모읍니다. **QR 스캔** 시 시간 제한 토큰 및 GPS 위치 정보를 검증하여 부정 스캔을 방지합니다.
...
2.  **실시간 통제**: **SSE(Server-Sent Events)** 기반으로 수신되는 실시간 공지/일정 알림을 통해 상황에 즉각 대응합니다.
3.  **트래픽 제어**: 인파 분산을 위해 제1전시장으로 향하는 팝업 퀘스트(경험치 2배)를 즉각 활성화하고, **Notification Hub**를 통해 전체 푸시 알림을 송출합니다.
4.  **현장 서비스 제공**: 트래픽이 몰리는 점심시간, 스태프가 태블릿의 **Meal Scanner** 화면을 열고 기기 카메라를 통해 참가자의 예약 QR을 0.5초 이내에 스캔/지급 처리합니다.
...
## 12. AR Navigation 구현
*   **WebXR API**: 모바일 디바이스 기반 증강현실 환경 구성.
*   **현재 위치 기반 경로 계산**: 카메라를 통한 객체 인식 및 사용자 위치 기반 목적지 경로 도출.
*   **AR 오버레이**: WebXR 세션 내 가이드 오버레이 구현.

## 13. i18n Pipeline 구현
*   **중앙화된 구조**: `src/i18n/locales` 내 네임스페이스별 분할 관리.
*   **LLM 자동 번역**: `scripts/translate.cjs`를 통한 로케일 동기화 및 자동 번역.
*   **검증 시스템**: `scripts/check-i18n-keys.cjs`를 통해 런타임 누락 키 방지.
...
## 17. 보안 텔레메트리 (Security Telemetry)
*   **실시간 모니터링**: 디바이스 상태 및 보안 로그 실시간 수집.
*   **단위 테스트**: `tests/unit/stores/securityStore.test.js`를 통해 로그 상태 변경 로직 검증.

## 18. 테스트 전략 (Test Strategy)
VX Web V2는 엔터프라이즈급 안정성을 확보하기 위해 다음 전략에 따라 테스트를 수행합니다.

### 1. 테스트 실행 방법
```bash
npm run test           # 전체 테스트 실행
npm run test run <path> # 특정 파일 테스트 실행
```

### 2. 테스트 범위 및 커버된 모듈
현재 `Vitest`와 `Vue Test Utils`를 사용하여 단위 테스트를 수행합니다.

| 분류       | 모듈명                           | 상태      |
| :--------- | :------------------------------- | :-------- |
| Store      | `mapStore`                       | 검증 완료 |
| Store      | `gamificationControlStore`       | 검증 완료 |
| Store      | `securityTelemetryStore`         | 검증 완료 |
| Store      | `hotelStore`                     | 검증 완료 |
| Store      | `operationalStore`               | 검증 완료 |
| Store      | `notificationStore`              | 검증 완료 |
| Store      | `notificationHubStore`           | 검증 완료 |
| Store      | `notificationControlStore`       | 검증 완료 |
| Store      | `questStore`                     | 검증 완료 |
| Store      | `deviceSettingsStore`            | 검증 완료 |
| Store      | `mealDistributionStore`          | 검증 완료 |
| Composable | `useArPermissions`               | 검증 완료 |
| Composable | `useNotificationControl`         | 검증 완료 |
| Composable | `useHotelManagement`             | 검증 완료 |
| Composable | `useSecurityTelemetry`           | 검증 완료 |
| Composable | `useMealDistribution`            | 검증 완료 |
| Service    | `hotelMockService`               | 검증 완료 |
| Service    | `mealDistributionMockService`    | 검증 완료 |
| Service    | `securityTelemetryMockService`   | 검증 완료 |
| Service    | `notificationControlMockService` | 검증 완료 |
| Router     | `router/index`                   | 검증 완료 |
| Router     | `router/routerGuard`             | 검증 완료 |
| Utils      | `i18nResolver`                   | 검증 완료 |
| Component  | `SpatialViewer`                  | 검증 완료 |
### 3. Mock Service 및 테스트 전략
*   **Pinia**: `setActivePinia(createPinia())`를 사용하여 스토어별로 독립적인 인스턴스를 생성하여 테스트합니다.
*   **LocalStorage**: `jsdom` 환경에서 `localStorage.clear()`를 통해 테스트 간 격리합니다.
*   **Dependencies**: 상위 의존성을 가진 스토어(예: `operationalStore`)는 `vi.mock`을 통해 모킹합니다.

### 4. PWA / WebXR 테스트 한계
현재 PWA 서비스 워커 및 WebXR 관련 테스트는 환경 제약으로 인해 Graceful Degradation(브라우저 API 미지원 시 안전한 fallback) 시나리오 위주로 검증하며, 실제 하드웨어 기반 검증은 별도의 수동 QA가 필요합니다.
